...
public class NumberInWords {
	...
	public static String convert(String num) {
		String inWords = NumberToWord(num);
		if (inWords.replaceAll(" Only", "").trim().length() == 0) {
			inWords = "Zero" + " Only";
		}
		return inWords.replaceAll("  ", " ").replaceAll("  ", " ");
	}
	...
	private static String NumberToWord(String number) {
		...
		if (len1 > split[split.length - 1]) {
			System.out.println("Maximum Allowed digits " + split[split.length - 1]);
			return "Exceeded One Hundred Crore";
		}
